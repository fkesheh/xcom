...
import type { CampaignState, CampaignWeaponId, OperationPlan } from "../campaign/types";
import { advanceGeoscape, interceptUfo } from "../campaign/geoscape";
...
import { Hud, type HudDebrief, type HudHover } from "./hud";
...
function requestedScreen(): string | null {
...
function showGeoscape(): void {
  ...
  geoscape = new GeoscapeView({
    ...
    onConfirmBase: (base) => {
      ...
      const campaign = current ?? createCampaign(base, newCampaignSeed());
    ...
    onInterceptUfo: () => {
      const current = loadCampaign();
      if (!current) return;
      const updated = interceptUfo(current);
      saveCampaign(updated);
      showGeoscape();
    },
    onResetCampaign: () => {
...
function showBase(campaign: CampaignState): void {
  ...
  baseView = new BaseView({
    ...
    onAssignWeapon: (soldierId: string, weaponId: CampaignWeaponId) => {
      const updated = assignSoldierWeapon(loadCampaign() ?? campaign, soldierId, weaponId);
      saveCampaign(updated);
      showBase(updated);
    },
    onToggleDeployment: (soldierId: string, deployed: boolean) => {
...
function startTactical(campaign: CampaignState, operation: OperationPlan = generateOperation(campaign)): void {
  ...
  if (
    ...
    deployment.length === 0 ||
    campaign.ufoContact?.status !== "crashed"
  ) {
    return;
  }
  geoscape?.dispose();
...
function openGeoscapeFromTactical(): void {
  const params = new URLSearchParams(window.location.search);
  params.set("screen", "geo");
...
window.addEventListener("keydown", (e: KeyboardEvent) => {
  switch (e.key) {
    ...
    case "Enter":
      ...
      break;
    case "Escape":
      if (hud.isBriefingOpen()) hud.toggleBriefing(false);
      else select(null);
      break;
    case "h":
    case "H":
...
window.addEventListener("keyup", (e: KeyboardEvent) => heldKeys.delete(cameraKey(e)));
...
if (existingCampaign && requestedScreen() !== "geo") showBase(existingCampaign);
else showGeoscape();
